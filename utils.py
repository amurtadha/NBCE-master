...
def filter_extremely_long_samples(df: pd.DataFrame, tokenizer: PreTrainedTokenizerBase, test=False) -> pd.DataFrame:
    ...
    if test:
        ...
        if ANSWER in df.columns:
            ...
            for answer in df[ANSWER]:
                concatenated_answers.extend(answer)
            max_ans= tokenizer(concatenated_answers, padding='longest',add_special_tokens=False, return_tensors='pt')['input_ids'].shape[-1]
        df[N_TOKENS] =  df[PROMPTS].map(lambda x: n_tokens_in_prompt(tokenizer, x)+max_ans)
    else:
