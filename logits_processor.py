...
class RestrictiveTokensLogitsProcessor(LogitsProcessor):
    ...
    #     self.mask = self.mask.tile(batch_size, 1)
    #     # print(self.mask.shape)
    #     # Make sure the batch size matches the restrictive token mask
    #     assert self.mask.shape[
    #                0] == batch_size, "Mismatch in batch size between input_ids and the restrictive token mask"
    #
    #     for batch_idx in range(batch_size):
    #         new_tokens_length = input_ids[batch_idx].shape[-1] - self.prompt_length_to_skip
    #         if new_tokens_length > 0:
    #             self.mask[batch_idx] = self.mask[batch_idx] & (
    #                         self.restrictive_token_ids[:, new_tokens_length - 1] == input_ids[
    #                     batch_idx, -1].item())
    #
    #     # Expand self.mask to match the shape of scores
    #     expanded_mask = self.mask.unsqueeze(1).unsqueeze(1)
    #
    #     # Apply the logits bias to the relevant tokens
    #     scores[expanded_mask] += self.logits_bias
    #
    #     return scores
