...
from constants import TEXT_BETWEEN_SHOTS, N_TOKENS, PROMPTS, ANSWER
...
class ExperimentManager:
    def __init__(self,dataset, test_df: pd.DataFrame, train_df: pd.DataFrame, model: NBCEModelWrapper,
    ...
    def _initialize_labels_and_logit_processor(self, labels: List[str]) -> None:
        ...
        if self.task not in ['piqa','copa', 'hellaswag', 'atis', 'arce']:
        ...
        # self.possible_labels = set(map_labels.keys())
    ...
    def get_few_shots_acc(self, windows_few_shot: List[str],  max_length:int) -> float:
        predicted_labels = self.get_predicted_labels(windows_few_shot, max_length=max_length)
        return self.calc_acc(predicted_labels)

    def get_predicted_labels(self, windows_few_shots: List[str], max_length:int) -> List[str]:
        windows_cache = self.model.get_contexts_cache(windows_few_shots, max_length=max_length)
        predicted_labels = []
        for index, row in self.test_df.iterrows():
            q = row[PROMPTS]
        # for q in self.test_df[PROMPTS]:
            a=None
            if self.task in ['piqa', 'hellaswag' ,'obqa', 'copa', 'arce']:
                a = row[ANSWER]
            predicted_label = self.predict_label(TEXT_BETWEEN_SHOTS + q, windows_cache, answer=a)
            predicted_labels.append(predicted_label)

        return predicted_labels

    def predict_label(self, task_text: str, cache: Dict, answer: str) -> str:
        assert task_text == task_text.rstrip(), "prompt ends with a space!"
        res = self.model.nbce_generate(task_text=task_text,
    ...
    def calc_acc(self, predicted_labels: List) -> float:
        if self.task in ['piqa', 'hellaswag','obqa', 'copa', 'arce']:
            true_labels= self.test_df[LABEL_TOKENS].values
            acc = len([t for i, t in enumerate(predicted_labels) if int(t)==int(true_labels[i])])/len(true_labels)
        else:
        ...
        for i, n_shots in enumerate(tqdm(n_shots_to_test)):
            ...
            while j < n_runs:
                ...
                if (longest_window_n_tokens + n_tokens_between_shots + self.test_df[N_TOKENS].max()
                    ...
                    continue
                # max_length=longest_window_n_tokens + n_tokens_between_shots + self.max_n_tokens -self.test_df[N_TOKENS].max()
                max_length=self.test_df[N_TOKENS].max()
                accuracies[i, j] = self.get_few_shots_acc(windows_few_shots, max_length=max_length)
                j += 1
        return accuracies
